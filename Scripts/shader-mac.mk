...
SHADERC := "$(THISDIR)../Tools/Bin/shaderc-macos"
...
vs_%.bin.h : vs_%.sc
	...
	$(SILENT) $(SHADERC) $(VS_FLAGS) \
		--platform linux -p 130 \
		-f "$<" -o "$(SHADER_TMP)" \
		--bin2c $(basename $(notdir $<))_glsl
	@cat "$(SHADER_TMP)" > "$@"

	-$(SILENT) $(SHADERC) $(VS_FLAGS) \
		--platform asm.js -p 300_es \
		-f "$<" -o "$(SHADER_TMP)" \
		--bin2c $(basename $(notdir $<))_essl
	-@cat "$(SHADER_TMP)" >> "$@"
